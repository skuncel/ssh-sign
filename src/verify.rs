use std::{process::{Command, Stdio, Output}, fs::File, collections::HashSet, io::{BufReader, BufRead}};
...
impl Verify {
  ...
  pub fn new(identity: Option<String>, file_path: String, signature_path: Option<String>, allowed_signers_path: Option<String>) -> Verify {
    if let Some(provided_signature_path) = signature_path {
      ...
      Verify {identity, file_path, signature_path: provided_signature_path, allowed_signers_path: resolved_allowed_signers_path }
    } else {
      let resolved_signature_path = format!("{}{}", file_path, ".sig");
      let resolved_allowed_signers_path = resolve_allowed_signers_path(allowed_signers_path);
      Verify {identity, file_path, signature_path: resolved_signature_path, allowed_signers_path: resolved_allowed_signers_path }
    }
  }
  ...
  fn run_verify_command(&self, identity: &String) -> Output {
    Command::new("ssh-keygen")
      .arg("-Y").arg("verify")
      .arg("-f").arg(&self.allowed_signers_path)
      .arg("-I").arg(identity)
      .arg("-n").arg("file")
      .arg("-s").arg(&self.signature_path)
      .stdin(Stdio::from(File::open(&self.file_path).expect("Couldn't open file to verify")))
      .stdout(Stdio::piped())
      .output()
  ...
  fn handle_verify_output(&self, output: Output) -> bool {
    if !output.stdout.is_empty() {
      ...
      println!("{}", String::from_utf8(output.stderr).expect("Failed read stderr"));
      return false;
    }
  }
  ...
  fn collect_allowed_signers_identities(&self) -> HashSet<String> {
    let allowed_signers_file = File::open(&self.allowed_signers_path).expect("Couldn't open allowed signers file");
    ...
    for line in allowed_signers_file_reader.lines() {
      let possible_identiy = self.get_identity_of_line(line.expect("Failed reading line in allowed signers file"));
